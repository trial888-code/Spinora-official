...
export async function loginToPanel(page: Page): Promise<void> {
  const panelUrl = process.env.FIREKIRIN_ADMIN_URL || "http://agent.firekirin.xyz:8580/admin/login";
  if (!page.url().includes("firekirin")) {
  ...
  if (await usernameInput.isVisible({ timeout: 3000 }).catch(() => false)) {
    const user = process.env.FIREKIRIN_AGENT_USER || "admin";
    const pass = process.env.FIREKIRIN_AGENT_PASS || "pass";
...
export async function createAccount(
  ...
  log("create", `Creating Fire Kirin user: ${username}`);
...
export async function rechargeAccount(page: Page, username: string, amount: number): Promise<void> {
  log("recharge", `Recharging Fire Kirin user ${username} with $${amount}`);
}

export async function redeemAccount(page: Page, username: string, amount: number): Promise<number> {
  log("redeem", `Redeeming Fire Kirin user ${username} $${amount}`);
  return amount;
}
